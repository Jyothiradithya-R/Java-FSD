...
public class Product extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/product","root","Bablu@3239");
			
			Statement st = con.createStatement();
			st.executeUpdate("use product");
			
			int res = st.executeUpdate("insert into product.product(id,Name,color,price)values('"+id_int+"','"+name+"','"+color+"','"+price_cost+"')");
			ResultSet rs = st.executeQuery("select * from product.product");
